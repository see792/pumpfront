<template>
	<view class="toptt">实时代币检测(钱包大于20分钟，第一次创币，推特优先级)</view>
	<view class="topBt">

		<button type="primary" class="opBt" size="mini" @click="tokenIncomeAnyseClick()">盈利分析</button>
			...
			<view v-for="(item, index) in dataList" :key="index" style="margin: auto;">
				...
				<view class="item-card" v-if="isShow(item)">
					<view class="item-header" :style="computedStyles(item)">
						<view class="head_title">
							<text class="abbreviation"
								@click="PumpClick( item.token )">{{ item.symbol }}(重复:{{item.symbol_repeat.length}})</text>
						...
						<view v-if="item.platform.length>0">
							<text style="font-size: 30px; color: red;font-weight: bold;text-decoration: underline;"
								@click="twitterSearch(item)">{{item.platform.toUpperCase()}}</text>
						</view>
						<view v-if="item.is_search||item.is_ray&&item.platform!='raypad'">
							<text style="font-size: 30px; color: red;font-weight: bold;text-decoration: underline;"
								@click="twitterTokenSearch(item)">推特CA</text>

						</view>
						<view v-if="item.is_twitter&&item.platform!='raypad'">
							<text style="font-size: 30px; color: red;font-weight: bold;text-decoration: underline;"
					...
					<view class="item-body">
						...
						<text v-if="item.is_twitter||item.is_search ||item.is_ray" class="inno-text"> 发推人:<text
								class="token"
						...
						<text v-if="item.is_twitter||item.is_search ||item.is_ray" class="inno-text"> 粉丝数:<text
						...
						<text class="inno-text"> 名字:<text
								class="token-nomarl">{{ item.name }}(重复:{{item.name_repeat.length}})</text></text>
						...
						<text class="inno-text"> 推特:<text class="token"
							...
							<text class="token" @click="blackTwitterClick2(item)" v-if="item.twitter.length>0">拉黑</text>
						</text>
						...
						<view class="holder-item">
							<view class="holder-item-left">
								...
								<view class="popBtView">
									...
									<button type="primary" class="popBt"
										@click="incomeClick(adddressInfo)">盈利分析</button>
								</view>
								<text></text>
							</view>
							<view class="holder-item-right">
								...
								<text class="holder-text" style="font-size: 20px;font-weight: bold;">10天代币:{{" "}}
									<text
										:style="computedStyles2(adddressInfo)">{{adddressInfo.ten_token_un_counts}}</text><text>/{{adddressInfo.ten_token_counts}}</text></text>
							</view>
				...
				<view class="popcontainer2">
					...
					<view class="item-card2">
						<view class="item-body">
							...
							<view style="margin-top: 10px;"><button type="primary" class="opBt" size="mini"
									style="margin: 0px;" @click="analyseIncomeToken(tkInfo.token)">前二十盈利</button>
							</view>
							...
							<view class="holder-item">
								<view class="holder-item-left">
									...
									<view class="popBtView">
										...
										<button type="primary" class="popBt" @click="incomeClick(holder)">盈利分析</button>
									</view>
								</view>
								<view class="holder-item-right">
									...
									<text class="holder-text" style="font-size: 20px;font-weight: bold;">10天代币:{{" "}}
										<text
											:style="computedStyles2(holder)">{{holder.ten_token_un_counts}}</text><text>/{{holder.ten_token_counts}}</text></text>
								</view>
	...
	<!-- 遮罩层和弹窗 -->
	<view v-if="showIncomeDialog" class="mask" @click="closeIncomeDialog">
		<view class="dialog2" @click.stop>
			<view class="dialog-header">
				<text>盈利分析</text>
				<button class="close-btn" @click="closeIncomeDialog">×</button>
			</view>
			<view class="dialog-body" style="padding: 1px;">
				<view class="popcontainer2">
					<view style="width: 2px;background-color: chocolate;"></view>
					<view class="item-card3">
						<scroll-view class="address-income-list" scroll-y="true">
							<view v-for="(income, index) in incomeInfo">
								...
								<view class="holder-item">
									<view class="holder-item-left">
										...
										<text class="token" style="font-size: 15px;margin: 0px;"
											@click="TokenClick(income.token)"><text
												style="color: black;">{{index+1+" "}}</text>{{income.token}}</text><text
											class="token" @click="copyClick(income.token)">复制</text>

									</view>
									<view class="holder-item-right">

										<text class="holder-text">盈利:<text
												style="color: red;">{{" "+income.income_sols.toFixed(6)}}SOL</text></text>

										<text class="holder-text" style="font-size: 20px;font-weight: bold;">倍率:{{" "}}
											<text>{{(income.rate*100).toFixed(4)}}%</text></text>
									</view>
	...
	<!-- 遮罩层和弹窗 -->
	<view v-if="showTokenIncomeDialog" class="mask" @click="closeTokenIncomeDialog">
		<view class="dialog2" @click.stop>
			<view class="dialog-header">
				<text>盈利分析</text>
				<button class="close-btn" @click="closeTokenIncomeDialog">×</button>
			</view>
			<view class="dialog-body" style="padding: 1px;">
				<view class="popcontainer2">
					<view style="width: 2px;background-color: chocolate;"></view>
					<view class="item-card3">
						<scroll-view class="address-income-list" scroll-y="true">
							<view v-for="(income, index) in tokenIncomeInfo">
								...
								<view class="holder-item">
									<view class="holder-item-left">
										...
										<text class="token" style="font-size: 15px;margin: 0px;"
											@click="OwnerClick(income.address)"><text
												style="color: black;">{{index+1+" "}}</text>{{income.address}}</text><text
											class="token" @click="copyClick(income.address)">复制</text>
	...
	export default {
		data() {
			return {
				...
				showTokenIncomeDialog: false,

				adddressInfo: {},
				tokenIncomeInfo: {},
				incomeInfo: {},
				tkInfo: {},
				Host: "http://panelapi.pumpfind.homes",
		...
		methods: {
			...
			incomeClick(addrInfo) {
				uni.showLoading({
					title: "分析中",
					mask: true
				})
				uni.request({
					url: this.Host + "/api/v1/test/income_detail?address=" + addrInfo.address, // 请求地址
					method: 'GET', // 请求方式
					timeout: 120000,
			...
			tokenIncomeAnyseClick() {
				uni.showModal({
					title: '盈利分析',
					content: '',
					placeholderText: "请输入代币地址",
					confirmText: "开始",
					editable: true,
					success: (res) => {
						if (res.confirm) {
							console.log('用户点击确定');
							console.log(res.content)
							this.analyseIncomeToken(res.content)
			...
			analyseIncomeToken(tk) {
				...
				uni.request({
					url: this.Host + "/api/v1/test/token_income_detail?address=" + tk, // 请求地址
					method: 'GET', // 请求方式
					timeout: 120000,
					success: (res) => { // 请求成功回调函数
						...
						if (res.data.code == 1) {
							...
							this.tokenIncomeInfo = res.data.data
							this.showTokenIncomeDialog = true
						} else {
							uni.showToast({
								title: res.data.msg
							})
			...
			computedStyles(it) {
				...
				if (it.is_twitter || it.is_search || it.is_ray) {
					return {
			...
			closeTokenIncomeDialog() {
				this.showTokenIncomeDialog = false;
			},
			closeTwitterDialog() {
