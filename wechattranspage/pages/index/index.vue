<template>
	<view class="toptt">微信代币检测</view>
	<view class="topBt">
		<button type="primary" class="opBt" size="mini" @click="buyOp(1,1)">开始购买(不格局)</button>
		<button type="primary" class="opBt" size="mini" @click="buyOp(1,0)">停止购买(不格局)</button>
		<button type="primary" class="opBt" size="mini" @click="buyOp(2,1)">开始购买(格局)</button>
		<button type="primary" class="opBt" size="mini" @click="buyOp(2,0)">停止购买(格局)</button>
		<button type="primary" class="opBt" size="mini" @click="addressClick()">微信管理</button>
				...
				<view v-for="(item, index) in dataList" :key="index" style="margin: auto;">
					...
					<view class="item-card" v-if="isShow(item)">
						...
						<view class="item-body">
							...
							<text class="inno-text"> 微信ID:<text
									class="token-nomarl">{{ getSenderInfo(item)}}</text></text>
	...
	<view v-if="showDialog" class="mask" @click="closeDialog">
		<view class="dialog" @click.stop>
			<view class="dialog-header">
				<text>微信管理</text>
				<view class="tibicview"> <label>微信id或者备注:</label> <input class="tibicviewinput" style="width: 400px;"
			...
			<view class="dialog-body">
				<view class="popcontainer">
					...
					<scroll-view class="address-list" scroll-y="true">
						...
						<view v-for="(address, index) in walletAddresses.slice(0, 200)" :key="address.id"
							...
							<view class="address-info">
								<view class="address-name">{{ address.memo }} </view>
								<view class="address">{{ address.weixin }}</view>
							...
							<view class="address-actions">
								...
								<button @click="lookAddress(address)" class="action-button">查看代币</button>
					...
					<button @click="openAddAddressModal" class="add-button">添加微信</button>

					<!-- 弹窗 -->
					<view v-if="showModal" class="modal">
						<view class="modal-content">
							<view class="modal-header">
								<text class="modal-title">{{ editingAddress ? '编辑微信' : '添加微信' }}</text>
							</view>
							<view class="modal-body">
								<input v-model="form.memo" class="input-field" placeholder="请输入微信备注" />
								<picker mode="selector" :range="addressTypesCnssss" :value="addressTypeIndex"
									@change="onAddressTypeChange">
									<view class="picker">
										<text>{{ addressTypesCn[form.type] || '请选择微信类型' }}</text>
									</view>
								</picker>

								<input v-model="form.weixin" class="input-field" placeholder="请输入微信id" />
	...
	<!-- 遮罩层和弹窗 -->
	<view v-if="showTokenDialog" class="mask" @click="closeTokenDialog">
		<view class="dialog" @click.stop>
			<view class="dialog-header">
				<text>微信代币管理</text>
				<button class="close-btn" @click="closeTokenDialog">×</button>
			</view>
			<view class="dialog-body">
				<view class="popcontainer">
					<!-- 钱包地址列表 -->
					<scroll-view class="address-list" scroll-y="true">
						<view v-for="(address, index) in weixinTokenList" :key="address.id" class="address-item">
							<view class="address-info">
								<view class="address-name">{{ index+1 }} </view>
								<view class="address token" style="color: blue;" @click="TokenClick(address.token)">
									{{ address.token }}
								</view>
								<view class="address">
									{{ formatDateTime(new Date(address.create_time*1000),"yyyy年MM月dd日 HH:mm:ss")}}
								</view>
								<view class="address">市值:{{address.market }}</view>
							</view>
...
<script>
	export default {
		data() {
			return {
				// 模拟数据
				addressTypes: ['user', 'clever'],
				addressTypesCnssss: ['普通用户', '聪明用户'],
				addressTypesCn: {
					"user": "普通用户",
					"clever": "聪明用户"
				},
				...
				adddressInfo: {},
				tkInfo: {},
				Host: "http://weixinapi.pumpfind.homes",
				showDialog: false, // 控制弹窗是否显示
				showTokenDialog: false, // 控制弹窗是否显示
		...
		methods: {
			getSenderInfo(item) {
				if (item.memo.length > 0) {
					if (item.is_sender_black) {
						return item.sender + "(" + item.memo + ",黑名单)"
					} else {
						return item.sender + "(" + item.memo + ")"
					}
				}else{
					if (item.is_sender_black) {
						return item.sender + "(黑名单)"
					} else {
						return item.sender
					}
				}
			...
			searchAddress() {
				...
				for (var j in this.oldWalletAddresssed) {
					if (this.oldWalletAddresssed[j].weixin.includes(this.vbsearch) || this.oldWalletAddresssed[j].memo
				...
				uni.request({
					...
					success: (res) => { // 请求成功回调函数
						if (res.data.code == 1) {
							...
							var logsrt = ["不格局机器人状态:" + st, "格局机器人状态:" + st2]
			...
			addressClick() {
				...
				uni.request({
					url: this.Host + "/api/v1/test/weixin", // 请求地址
					method: 'POST', // 请求方式
					success: (res) => { // 请求成功回调函数
			...
			saveAddress() {
				console.log(this.form)
				if (!this.form.memo || !this.form.type || !this.form.weixin) {
					uni.showToast({
						title: '请填写所有字段',
						icon: 'none'
					});
				...
				if (this.editingAddress !== null) {
					...
					console.log(this.form)
					uni.request({
						url: this.Host + "/api/v1/test/change_weixin", // 请求地址
						method: 'POST', // 请求方式
						header: {
							'content-type': 'application/json' // 根据后端要求设置合适的Content-Type
						},
					...
					uni.request({
						url: this.Host + "/api/v1/test/add_weixin", // 请求地址
						method: 'POST', // 请求方式
						header: {
							'content-type': 'application/json' // 根据后端要求设置合适的Content-Type
						},
			...
			lookAddress(index) {
				...
				uni.request({
					url: this.Host + "/api/v1/test/weixin_token_list?weixin=" + index.weixin, // 请求地址
					method: 'GET', // 请求方式
					success: (res) => { // 请求成功回调函数
						this.weixinTokenList = res.data.data
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							...
							uni.request({
								url: this.Host + "/api/v1/test/delete_weixin", // 请求地址
								method: 'POST', // 请求方式
