				...
				<view class="item-card" v-if="isShow(item)">
					<view class="item-header" :style="computedStyles(item)">
						<view class="head_title">
							<text class="abbreviation" @click="PumpClick( item.token )">{{ item.symbol }}(重复:{{item.symbol_repeat.length}})</text>
						...
						<view v-if="item.is_search||item.is_ray">
					...
					<view class="item-body">
						...
						<text v-if="item.is_twitter||item.is_search ||item.is_ray" class="inno-text"> 发推人:<text class="token"
						...
						<text class="inno-text"> 名字:<text class="token-nomarl">{{ item.name }}(重复:{{item.name_repeat.length}})</text></text>
						...
						<text class="inno-text"> 发币数:<text class="token-nomarl">{{ item.tk_count }}</text><text class="token"
										@click="lookTokenClick(item.owner)">查看</text></text>
						...
						<text class="inno-text"> 溯源钱包:<text class="token"
								@click="OwnerClick( item.su_address)">{{ dealSuFrom(item)+"("+item.su_level+"层)"}}</text><text class="token"
								@click="copyClick(item.su_address)">复制</text></text>
