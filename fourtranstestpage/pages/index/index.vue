<template>
	<view class="toptt">实时代币检测(前十小草，6，前五小草3)</view>

	<view class="topBt">
		<button type="primary" class="opBt" size="mini" @click="buyOp(1,1)">开始购买</button>
		<button type="primary" class="opBt" size="mini" @click="buyOp(1,0)">停止购买</button>
						...
						<view v-for="(holder ,index) in tkInfo.list">
							...
							<view class="holder-item">
								...
								<view class="holder-item-right">
									...
									<text class="holder-text">30天成本:<text
											style="color: red;">{{" "+holder.aven_sol.toFixed(2)}}SOL</text></text>
									...
									<text class="holder-text" style="font-size: 20px;font-weight: bold;">30天代币:{{" "}}
										<text
	...
	export default {
		data() {
			return {
				...
				adddressInfo: {},
				tkInfo: {},
				Host: "http://fourtranstestapi.pumpfind.homes",
		...
		methods: {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							...
							this.analyseToken(res.content, 0)
			...
			analyseToken(tk, mk) {
				...
				uni.request({
					url: this.Host + "/api/v1/test/detail?address=" + tk + "&mk=" + mk, // 请求地址
					method: 'GET', // 请求方式
					timeout: 120000,
					success: (res) => { // 请求成功回调函数
			...
			updateInfo() {
				...
				for (let j in this.dataList) {
					if (this.dataList[j].image.length == 0) {
						uni.request({
							url: "https://four.meme/meme-api/v1/private/token/get/v2?address=" + this.dataList[j]
								.token, // 请求地址
							method: 'GET', // 请求方式
							success: (res) => { // 请求成功回调函数
								if (res.data.code == 0 && res.data.data != null) {
									this.dataList[j].image = res.data.data.image
									
									if(res.data.data.webUrl){
										this.dataList[j].website = res.data.data.webUrl
										
									}
									if(res.data.data.telegramUrl){
										this.dataList[j].telegram = res.data.data.telegramUrl
										
									}
									if(res.data.data.twitterUrl){
										this.dataList[j].twitter = res.data.data.twitterUrl
			...
			TokenClick: function(it) {
				window.open("https://gmgn.ai/bsc/token/" + it)
			},
			...
			OwnerClick: function(it) {
				console.log(it)
				window.open("https://bscscan.com/address/" + it)
			},
			HashClick: function(it) {
				console.log(it)
				window.open("https://bscscan.com/tx/" + it)
			},
			PumpClick: function(it) {
				console.log(it)
				window.open("https://four.meme/token/" + it)
			},
